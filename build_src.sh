...
if [ -z "$helix_VERSION" ] || [ -z "$BUILD_VERSION" ]; then
    ...
    echo "Example: $0 25.07.1 1"
    exit 1
fi

PACKAGE_NAME="helix"
ORIG_TARBALL="${PACKAGE_NAME}_${helix_VERSION}.orig.tar.gz"
BUILD_DIR="${PACKAGE_NAME}-${helix_VERSION}"

echo "Creating Debian/Ubuntu source packages for helix ${helix_VERSION}-${BUILD_VERSION}..."
...
if [ ! -f "$ORIG_TARBALL" ]; then
    ...
    wget -q "https://github.com/helix-editor/helix/archive/refs/tags/${helix_VERSION}.tar.gz" -O "$ORIG_TARBALL"
...
build_source_package() {
    local dist=$1
    local FULL_VERSION="${helix_VERSION}-${BUILD_VERSION}~${dist}"
    ...
    rm -rf "$BUILD_DIR"
    tar -xf "$ORIG_TARBALL"
    # GitHub archives extract as helix-25.x.y/ which matches our BUILD_DIR
...
helix (${FULL_VERSION}) ${dist}; urgency=medium

  * New upstream release ${helix_VERSION}.
